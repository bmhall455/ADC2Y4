...
# ------------------------------------------------------------------
# Helper: Truncate goal lane centerline with minimum fraction enforcement.
# ------------------------------------------------------------------
# def truncate_goal_lane(goal_lane_center, goal_gps, min_fraction=0.50):
#     """
#     Truncates the goal lane's centerline so that it ends at goal_gps.
#     It computes cumulative distances along the lane. Then it first finds the
#     point in the centerline that is closest to goal_gps by geodesic distance.
#     If that point occurs before min_fraction of the total lane length has been traveled,
#     it forces the truncation to occur later.
#     """
...
#     if total_length == 0:
#         rospy.logwarn("Zero total length in goal lane; using goal GPS only.")
#         return [goal_gps]
    
#     # Find candidate index (closest point to goal)
#     min_dist = float('inf')
#     candidate_idx = 0
#     for i, pt in enumerate(goal_lane_center):
#         d = geodesic((goal_gps[1], goal_gps[0]), (pt[1], pt[0])).meters
#         if d < min_dist:
#             min_dist = d
#             candidate_idx = i
...
def truncate_goal_lane(goal_lane_center, goal_gps, min_fraction, threshold):
    """
    Truncates the goal lane's centerline so that it ends at goal_gps.
    It iterates over the lane points and finds the first point whose cumulative
    fraction along the lane is at least min_fraction and whose geodesic distance
    to goal_gps is less than the specified threshold (in meters).

    Parameters:
      goal_lane_center: list of [lon, lat] pairs representing the goal lane.
      goal_gps: [lon, lat] of the desired endpoint.
      min_fraction: minimum fraction (0 - 1) of the lane's total length that must be covered.
      threshold: maximum allowable distance (meters) from a lane point to goal_gps to consider it valid.

    Returns:
      A truncated list of [lon, lat] pairs ending exactly at goal_gps.
    """
    import rospy
    from geopy.distance import geodesic
